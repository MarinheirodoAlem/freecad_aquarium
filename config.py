...
def DefaultsConfig(categories):
    ...
    #defcon.addNum('ColumnsStandLengthCount', 0, 0, 10, 1, 'Number of extra columns along the length of the stand', num, 'structure')
    ...
    defcon.addNum('JunctionThickness', 1.5, 0, 5, .5, 'Spacing between panel joints (silicone thickness or none for acrylic glue)', mm, 'glass', 'weir')
    ...
    defcon.addNum('WeirWallThickness', 5, 1, 10, 1, 'Thickness of the weir panel', mm, 'weir')
    defcon.addNum('WeirSlotWidth', 3, 1, 10, 1, 'Size of the slot in the weir that lets water in and keep snail out', mm, 'weir')
    defcon.addNum('WeirFastenerDiameter', 6, 1, 10, 1, 'Diameter of holes to fasten weir', mm, 'weir')
    defcon.addNum('WeirMountHoleDiameter', 5, 1, 10, 1, 'Diameter of holes before tapping, to screw the water level panel', mm, 'sump')
    defcon.addNum('WeirFastenerOffset', 10, 5, 50, 1, 'Space from glass to center of hole used to fasten the weir', mm, 'weir')
    defcon.addNum('WeirFastenerHorizontalCount', 7, 2, 50, 1, 'Number of horizontal holes to fasten weir on each line', num, 'weir')
    defcon.addNum('WeirFastenerVerticalCount', 4, 2, 50, 1, 'Number of vertical holes to fasten weir on each line', num, 'weir')
    ...
    defcon.addNum('SumpBraceWidth', 20, 1, 100, 1, 'Size of braces', mm, 'sump')
    defcon.addNum('SumpBraceFillet', 50, 1, 100, 1, 'Rounding of acrylic edges on top of sump', mm, 'sump')
    defcon.addNum('SumpHeight', 350, 10, 500, 300, 'Height of sump', mm, 'sump')
    defcon.addNum('SumpExtraSpaceForChiller', 350, 10, 1000, 1, 'Space left inside sump for extra dry equipment (like chiller)', mm, 'sump')
    defcon.addNum('SumpMarginLeft', 5, 1, 50, 1, 'Space on left of sump to the structure', mm, 'sump')
    defcon.addNum('SumpMarginRight', 5, 1, 50, 1, 'Space on right of sump to the structure', mm, 'sump')
    defcon.addNum('SumpMarginFront', 60, 1, 50, 1, 'Space on front of sump to the structure', mm, 'sump')
    defcon.addNum('SumpMarginBack', 5, 1, 50, 1, 'Space on back of sump to the structure', mm, 'sump')
    ...
    defcon.addNum('FugeSlotHeight', 15, 5, 50, 1, 'Height refugium that lets water out and keep algae inside', mm, 'sump')
    ...
    defcon.addNum('FugeFillet', 3, 1, 10, 1, 'Rounding of acrylic edges', mm, 'sump')
    defcon.addNum('FugeCompartments', 2, 1, 10, 1, 'Number of Refugiums/Algae reactors in sump', num, 'sump')
    defcon.addNum('FugeWeirFastenerDiameter', 6, 1, 10, 1, 'Diameter of holes to fasten refugium weir', mm, 'sump')
    defcon.addNum('FugeWeirFastenerVerticalCount', 4, 2, 10, 1, 'Number of vertical holes to fasten weir on each line of refugium', num, 'sump')
    defcon.addNum('FugeWeirSlotsVerticalCount', 2, 2, 10, 1, 'Number of vertical slots used for weir level positioning', num, 'sump')
    return defcon
...
def MakeComputed(doc):
    ...
    s.add('BeamsDir', '=Width>Length?0:1')
    ...
    s.add('RightCornerX', '=+Width/2', 'Coordinate of corner')
    s.add('LeftCornerX', '=-Width/2', 'Coordinate of corner')
    s.add('FrontCornerY', '=-Length/2', 'Coordinate of corner')
    s.add('BackCornerY', '=+Length/2', 'Coordinate of corner')
    s.add('RightCornerXSump', '=RightCornerX-(Config.SumpMarginRight+Config.MetalProfileHeight)')
    s.add('LeftCornerXSump', '=LeftCornerX+(Config.SumpMarginLeft+Config.SumpExtraSpaceForChiller+Config.MetalProfileHeight)')
    s.add('FrontCornerYSump', '=FrontCornerY+(Config.SumpMarginFront+Config.MetalProfileHeight)', 'Coordinate of corner')
    s.add('BackCornerYSump', '=BackCornerY-(Config.SumpMarginBack+Config.MetalProfileHeight)', 'Coordinate of corner')
    s.add('SumpWidth', '=Width-(Config.SumpMarginLeft+Config.SumpMarginRight+2*Config.MetalProfileHeight)')
    s.add('SumpLength', '=Length-(Config.SumpMarginFront+Config.SumpMarginBack+2*Config.MetalProfileHeight)')
    s.add('SumpWetWidth', '=SumpWidth-Config.SumpExtraSpaceForChiller')
    s.add('BeamsSumpSizeWidth', '=(Width-(2+Config.ColumnsStandWidthCount)*Config.MetalProfileWidth)/(Config.ColumnsStandWidthCount+1)', 'Length of beams in the width size')
    s.add('BeamsSumpSizeLength', '=Length-2*Config.MetalProfileHeight', 'Length of beams in the length size')
    s.add('BeamsSizeWidth', '=Width-Config.MetalProfileWidth', 'Length of beams in the width size')
    s.add('BeamsSizeWidth45', '=Width', 'Length of beams in the width size')
    s.add('BeamsSizeLength', '=Length', 'Length of beams in the length size')
    s.add('BeamsSizeMiddle', '=min(Width;Length)-Config.MetalProfileWidth', 'Length of beams in the width size')
    s.add('SumpBeamSpacing', '=((BeamsDir>0?Length:Width)-Config.MetalProfileHeight)/max(1;Config.BeamsSumpCount+1)')
    s.add('StandBeamSpacing', '=((BeamsDir>0?Length:Width)-Config.MetalProfileHeight)/max(1;Config.BeamsStandCount+1)')
    ...
    s.add('NozzlesCount', '=1+((BeamsDir>0?Length:Width) - NozzlesStart - NozzlesEnd) / max(1; abs(StandBeamSpacing*Config.NozzlesEveryXBeamSpace))')
    s.add('NozzlesSpacingRows', '=(BeamsDir>0?Width:Length-WeirDepth)/max(Config.NozzlesRows;1)')
    ...
    s.add('FlangesY', '=Length/2-WeirCenter-Config.SidesGlassThickness', 'Center of flanges, Y')
    ...
    s.add('WeirWidth', '=Width-2*WeirMargin')
    s.add('WeirInsideWidth', '=WeirWidth-2*Config.SidesGlassThickness')
    s.add('WeirHorizontalFastenerHidth', '=WeirInsideWidth-2*Config.WeirFastenerOffset')
    ...
    s.add('BulkHeadZLevel', '=RealGlassHeight-Config.BulkHeadDiameter/2-Config.SidesGlassThickness-Config.WeirSlotWidth*1.5')
    s.add('FlangeCount', '0')
    s.add('WeirFlangeOffset', '=(WeirInsideWidth-FlangesMaxDiameter)/max(FlangeCount-1;1)')
    s.add('BeamCanopyFront2BackLength', '=Length-2*Config.CanopyProfileHeight')
    s.add('BeamCanopyLeft2RightLength', '=Width-2*Config.CanopyProfileWidth')
    ...
    s.add('CanopySpacingX', '=(Width - Config.CanopyProfileHeight) / max(1; Config.CanopyExtraBeams + 1)')
    ...
    s.add('SumpCompartmentsLength', '=SumpLength-Config.SumpAcrylicThickness')
    s.add('SumpCompartmentLength', '=SumpCompartmentsLength/Config.FugeCompartments')
    s.add('FugeWeirDoorHeight', '=(Config.SumpHeight-Config.SumpAcrylicThickness)/2-Config.FugeBorder')
    s.add('FugeWeirHoleMargin', '=Config.FugeBorder/2')
    s.add('FugeWeirFastenerVerticalHeight', '=FugeWeirDoorHeight')
    s.add('FugeWeirSlotVerticalFullSpace', '=((Config.SumpHeight-Config.SumpAcrylicThickness)/2-FugeWeirHoleMargin)')
    s.add('FugeWeirSlotVerticalOffset', '=FugeWeirSlotVerticalFullSpace*(Config.FugeWeirSlotsVerticalCount-1)/Config.FugeWeirSlotsVerticalCount+Config.FugeWeirFastenerDiameter')
    s.add('FugeWeirSlotVerticalHeight', '=(FugeWeirSlotVerticalFullSpace-2*Config.FugeWeirFastenerDiameter*(Config.FugeWeirSlotsVerticalCount-1))/Config.FugeWeirSlotsVerticalCount')
    s.add('SumpCompartmentHoleSpacing', '=SumpCompartmentLength-Config.SumpAcrylicThickness-(Config.FugeSlideSpacing+FugeWeirHoleMargin)')
    s.add('SumpPanelsHeight', '=Config.SumpHeight-2*Config.SumpAcrylicThickness')
    s.recompute()
